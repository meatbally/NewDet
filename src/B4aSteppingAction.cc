...
// Collect energy and track length step by step
  ...
  G4double stepTime = step->GetDeltaTime();
  G4double stepLength = step->GetStepLength();

  G4Track* track     = step->GetTrack();
  G4double kinEnergy = track->GetKineticEnergy();
  
  if ( volume == fDetConstruction->GetTest1PV() ) fEventAction->FindEnergy1(kinEnergy);
  if ( volume == fDetConstruction->GetTest2PV() ) fEventAction->FindEnergy2(kinEnergy);
}
